...
$checksum = new Checksum($input);
echo "Part 1: " . $checksum->getChecksum();
echo "<br />";
echo "Part 2: " . $checksum->getDivisableSum();
...
class Checksum
{
    ...
    public function getChecksum()
    {
        ...
        foreach ($this->input as $input) {
            $sum += $this->getMaxMinDiff($input);
        }

        return $sum;
    }

    private function getMaxMinDiff($input)
    {
        $max = 0;
        $min = $input[0];
    ...
    public function getDivisableSum()
    {
        $sum = 0;

        foreach ($this->input as $input) {
            $sum += $this->getDivisedDevisable($input);
        }

        return $sum;
    }

    private function getDivisedDevisable($input, $currentKey = 0)
    {
        $position = 0;

        if ($currentKey < sizeof($input)) {
            foreach ($input as $digit) {
                if ($position !== $currentKey) {
                    $devidedValue = $input[$currentKey] / $digit;

                    if ($devidedValue == floor($devidedValue)) {
                        return $devidedValue;
                    }
                }
        ...
        return $this->getDivisedDevisable($input, ++$currentKey);
    }
}